/*
 * libwifi.h - parse-only subset for pycom firmware integration.
 *
 * The upstream libwifi.h pulls in gen/ (frame generation helpers) which
 * are not required for our use case (we only parse promiscuous-mode captures).
 * gen/ is therefore not vendored, and this header lists only parse + core.
 */
#ifndef LIBWIFI_H
#define LIBWIFI_H
